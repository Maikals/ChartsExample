 ...
 */
package com.example.miquelcastanys.chartsexample.Adapters;
...
public class ChartAdapter {
    ...
    public ChartAdapter(BarChart chart) {
        this.chart = chart;
    }
    ...
    public void setData(ChartValues[] chartValues) {
        ...
        for (int i = 0; i < chartValues.length; ++i) {
            BarEntry entry = new BarEntry(chartValues[i].getX(), chartValues[i].getY());
            entries.add(entry);
        }
        BarDataSet barDataSet = new BarDataSet(entries, "TEST GRAPH");
