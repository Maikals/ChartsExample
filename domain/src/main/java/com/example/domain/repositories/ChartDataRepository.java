...
import com.example.domain.ws.RestCall;
import com.google.gson.Gson;

public class ChartDataRepository {

    public ChartDataDomainEntity getChartData(String url) {
        RestCall restCall = new RestCall();
        String data = restCall.getJSONStringFromURLGet(url);
        if (data == null) return null;
        Gson gson = new Gson();
        try {
            return gson.fromJson(data, ChartDataDomainEntity.class);
        } catch (Exception e) {
            throw e;
        }
    }
